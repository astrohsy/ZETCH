...
class LocationServiceIntegrationTest {
    ...
    void update() {
        locationRepository.save(l1);
        LocationEntity newLocation = locationService.update(NAME_1, "Alex's", "Mediterranean", "5678 Broadway", "museum");
    ...
    void assignOwnerFails() {
        assertThrows(
            NoSuchElementException.class,
            () -> locationService.assignOwner(NAME_1, "cat"));
    }
    ...
    void getRatingHistogramFails() {
        assertThrows(
            NoSuchElementException.class,
            () -> locationService.getRatingHistogram(NAME_1));
    }

    @Test
    void createNew() {
        LocationEntity newLocation = locationService.createNew("Alex's", "Mediterranean", "5678 Broadway", "museum");
        assertThat(locationService.getAll().size(), is(1));
        assertThat(newLocation.getName(), is("Alex's"));
        assertThat(newLocation.getDescription(), is("Mediterranean"));
        assertThat(newLocation.getAddress(), is("5678 Broadway"));
    ...
    void averageRatingFails() {
        assertThrows(
            NoSuchElementException.class,
            () -> locationService.averageRating(NAME_1));
    }
}
